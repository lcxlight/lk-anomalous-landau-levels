...
\end{align}
Note that $m^*_{\mathrm{eff},\uparrow-} \propto B$ in the bulk regime ($\dmu^2\ll c^2B$), while $m^*_{\mathrm{eff},\downarrow-}\approx c^2B/[2a(\mu-E_0)^2] \propto B/(\mu-E_0)^2$, both strongly field-dependent in contrast to the constant mass $m^*_{\mathrm{eff},\alpha +}=1/(2a)$ of the dispersive upper branches.
At fixed carrier density $\rho$, the chemical potential $\mu_{\rho}(B)$ is determined self-consistently; plugging this trajectory into the above formulas yields the dashed theory curves in Fig.~\ref{fig:main4} of the main text.
